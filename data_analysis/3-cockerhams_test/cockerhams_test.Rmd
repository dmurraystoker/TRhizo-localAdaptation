...
# Rural
aboveground.biomass.rural.data <- biomass.data %>%
  select(Population, Microbiome, Block, Aboveground_Biomass) %>%
  filter(Microbiome == "Nonlocal_R")
# Urban
aboveground.biomass.urban.data <- biomass.data %>%
  select(Population, Microbiome, Block, Aboveground_Biomass) %>%
...
# Rural
aboveground.biomass.rural.Vg.LMM <- lmer(
  sqrt(Aboveground_Biomass) ~ Block + (1 | Population),
  data = aboveground.biomass.rural.data,
  REML = TRUE
)

aboveground.biomass.rural.Vg.df <- data.frame(VarCorr(aboveground.biomass.rural.Vg.LMM)) %>%
  filter(grp == "Population") %>%
  add_column(Microbiome = "Rural") %>%
  add_column(Nitrogen = "Combined")

# Urban
aboveground.biomass.urban.Vg.LMM <- lmer(
  sqrt(Aboveground_Biomass) ~ Block + (1 | Population),
  data = aboveground.biomass.urban.data,
  REML = TRUE
)

aboveground.biomass.urban.Vg.df <- data.frame(VarCorr(aboveground.biomass.urban.Vg.LMM)) %>%
  filter(grp == "Population") %>%
...
aboveground.biomass.combined.Vg.data <- bind_rows(
  ...
  aboveground.biomass.rural.Vg.df,
  aboveground.biomass.urban.Vg.df
)
...
write_csv(combined.Vg.data, file = "data/cockerham_data_uncleaned.csv")
```
...
# Local by Rural data
nodule.density.local.by.rural.data <- full.data %>%
  filter(Microbiome != "Nonlocal_U") %>%
...
# Local by Urban data
nodule.density.local.by.urban.data <- full.data %>%
  filter(Microbiome != "Nonlocal_R") %>%
  group_by(Population, Microbiome, Nitrogen) %>%
  summarise(
    Nod_Density = mean(Nodule_Density),
...
# Rural by Urban data
nodule.density.rural.by.urban.data <- full.data %>%
  filter(Microbiome != "Local") %>%
  group_by(Population, Microbiome, Nitrogen) %>%
  summarise(
    Nod_Density = mean(Nodule_Density),
...
```{r Local by Rural Correlations, echo = FALSE}
kable(
  cor(local.by.rural.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between local and nonlocal-rural for aboveground biomass and fixing nodule density."
) %>%
...
```{r Local by Urban Correlations, echo = FALSE}
kable(
  cor(local.by.urban.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between local and nonlocal-urban for aboveground biomass and fixing nodule density."
) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Rural by Urban Correlations, echo = FALSE}
kable(
  cor(rural.by.urban.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between nonlocal-rural and nonlocal-urban for aboveground biomass and fixing nodule density."
) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Nodule Density: Local by Rural Correlations, echo = FALSE}
kable(
  cor(nodule.density.local.by.rural.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between local and nonlocal-rural by nitrogen treatment for nodule density."
) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Nodule Density: Local by Urban Correlations, echo = FALSE}
kable(
  cor(nodule.density.local.by.urban.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between local and nonlocal-urban by nitrogen treatment for nodule density."
) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Nodule Density: Rural by Urban Correlations, echo = FALSE}
kable(
  cor(nodule.density.rural.by.urban.data %>% select(-1), method = "pearson"),
  booktabs = TRUE,
  digits = 3,
  caption = "Fitness correlations between nonlocal-rural and nonlocal-urban by nitrogen treatment for nodule density."
) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
cockerham.data <- read_csv(
  "data/cockerham_data_cleaned.csv",
  col_types = c("ffnnn"),
...
save.image("data_analysis/3-cockerhams_test/cockerhams_test-workspace.RData")
```
